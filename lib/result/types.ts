...
export type CalculationResult = {
   ...
   *  - "mellomledd": delsteg, faktor, geometri eller kapasitet brukt undervegs
   *  - "input": ein gitt inngangsverdi som berre er ekko-a
   * Valfri — eldre køyringar manglar feltet, og tile-heuristikken fell
   * då tilbake til den regelbaserte gjettinga.
   */
  result_roles?: Record<string, "dimensjonerande" | "mellomledd" | "input">;
  limitations: string[];
  warnings: string[];
...
export type ComparisonResult = {
  ...
  summary: string;
  /**
   * Nøklar rapportert av berre éin konstruktør. Sett deterministisk av
   * Samanliknar-routen (kode, ikkje LLM). Slike nøklar er IKKJE avvik og
